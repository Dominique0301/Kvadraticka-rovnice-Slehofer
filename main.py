...
class Application(tk.Tk):
    ...
    def __init__(self):
        ...
        self.fr = tk.LabelFrame(self, text="Výpočty", bd=4)
        self.fr.grid(row=1, column=2)

        self.fr2 = tk.LabelFrame(self, text="Graf", bd=4)
        self.fr2.grid(row=1, column=4)
        

        #vypsání diskriminantu
        self.d=None
        self.w = 1
        self.lbl = tk.Label(self.fr, text="")
        self.lbl.grid(row=10, column=2)
        self.lbl_d = tk.Label(self.fr, text="D =")
        self.lbl_d.grid(row=10, column=1)

       #vypsání výsledků 
        self.lblx1 = tk.Label(self.fr, text="")
        self.lblx1.grid(row=11,column=2)
        self.lblx2 = tk.Label(self.fr, text="")
        self.lblx2.grid(row=12,column=2)
        self.lbl_x1 = tk.Label(self.fr, text="X1 =")
        self.lbl_x1.grid(row=11, column=1)
        self.lbl_x2 = tk.Label(self.fr, text="X2 =")
        self.lbl_x2.grid(row=12, column=1)

        #vstup a
        self.lbla = tk.Label(self.fr, text="a =")
        self.lbla.grid(row=2, column=1)  
        self.a_vstup = tk.Entry(self.fr,validate="key", validatecommand=(self.register(self.validate), "%P"))
        self.a_vstup.grid(row=2, column=2, padx=10, pady=10)

        #vstup b
        self.lblb = tk.Label(self.fr, text="b =")
        self.lblb.grid(row=4, column=1)  
        self.b_vstup = tk.Entry(self.fr,validate="key", validatecommand=(self.register(self.validate), "%P"))
        self.b_vstup.grid(row=4, column=2, padx=10, pady=10)

        #vstup c
        self.lblc = tk.Label(self.fr, text="c =")
        self.lblc.grid(row=6, column=1)  
        self.c_vstup = tk.Entry(self.fr, validate="key", validatecommand=(self.register(self.validate), "%P"))
        self.c_vstup.grid(row=6, column=2, padx=10, pady=10)

        self.lblGraf=tk.Button(self.fr2, text="Zobrazit graf",command=self.graf)
        self.lblGraf.grid(row=8,column=3)
        self.btn=tk.Button(self.fr, text="Diskriminant", command=self.validace)
        self.btn.grid(row=8,column=2)
        self.btn2=tk.Button(self.fr, text="Výsledky", command=self.validace_x)
        self.btn2.grid(row=9, column=2)

        #nastavení pro graf
        self.mr = IntVar()
        self.m = tk.Checkbutton(self.fr2, text="Mřížka grafu", variable=self.mr)
        self.m.grid(row=2, column=4)
        self.x_label = tk.Entry(self.fr2)
        self.x_label.grid(row=3, column=4, padx=10, pady=10)
        self.y_label = tk.Entry(self.fr2)
        self.y_label.grid(row=4, column=4,padx=10, pady=10)
        self.xlbl = tk.Label(self.fr2, text="Popis osy X")
        self.xlbl.grid(row=3, column=3)
        self.ylbl = tk.Label(self.fr2, text="Popis osy Y")
        self.ylbl.grid(row=4, column=3)
        self.title = tk.Entry(self.fr2)
        self.title.grid(row=5,column=4,padx=10, pady=10)
        self.title_lbl = tk.Label(self.fr2, text="Název grafu")
        self.title_lbl.grid(row=5, column=3)
        self.w = tk.Entry(self.fr2, validate="key", validatecommand=(self.register(self.kontrola_tloustky),"%P"))
        self.w.grid(row=6, column=4,padx=10, pady=10)
        self.wlbl=tk.Label(self.fr2, text="Tloušťka grafu")
...
#zvolí se vhodný popstup řešení
    ...
    def graf(self):
        if self.d == None:
             messagebox.showerror("Chyba", "Nemáte výsldky, které lze zobrazit")
        elif self.d > 0:
            self.graf_dve()
        else:
            pass
